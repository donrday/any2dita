...
<!-- markup conversion: map elements -->
	...
	<xsl:template match='*[contains(@class," map/xtopicref ")]'>
			<a>
				<xsl:attribute name='href'><xsl:value-of select='$queryPath'/><xsl:value-of select='@href'/></xsl:attribute>
				<xsl:if test='@scope'><xsl:attribute name='target'><xsl:value-of select='@scope'/></xsl:attribute></xsl:if>
				<xsl:if test="@id"><xsl:attribute name="id"><xsl:value-of select="@id"/></xsl:attribute></xsl:if>
				<xsl:choose>
