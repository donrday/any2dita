...
<!-- markup conversion: map elements -->
	...
	<xsl:template match='topic' mode='passthru'>
		...
		<xsl:variable name="fullPath">
			<xsl:value-of select="concat('content/',$groupName,'/',@href)"/>
		</xsl:variable>
...
<xsl:text>
			...
			<div class="step slide" data-x="{position()}000" data-y="{$yoffset - 3000 + ($seqnum * 300)}" data-rotate="{(position() - 4) * 23}" 
		 	data-z="-{$seqnum * 1000}" 
			data-scale="0.{10 - $topicref_nesting}">
					<!-- Adding ?file=value turns the link into a parameter for the GET event processor. -->
