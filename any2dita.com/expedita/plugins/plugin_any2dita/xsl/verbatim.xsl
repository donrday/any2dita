...
<!--  parameter for building expeDITA links -->
...
<xsl:template mode="verb" match="@*">
  ...
  <xsl:value-of select="concat(' ',name(.),'=')"/>
  <xsl:text>"</xsl:text>
  <!-- DRD: capture attval as a var; if = href then link it -->
  ...
  <xsl:choose>
  	<xsl:when test="name(.) = 'href'">
  		<a href="{$patchpath}{$targPath}/{$attval}" target="_blank"><xsl:value-of select="$attval"/></a>
  	</xsl:when>
  	<xsl:otherwise>
